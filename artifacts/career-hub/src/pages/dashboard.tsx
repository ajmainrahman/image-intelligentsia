...
import { AlertTriangle, Clock, Briefcase, CircleCheckBig, Clock3, XCircle, Sparkles, CheckCircle2 } from "lucide-react";
...
function StatCard({ label, value, tone, sub, href }: { label: string; value: string | number; tone: string; sub?: string; href: string }) {
  return <Link href={href}><div className="rounded-[24px] border border-[#e4ddd2] bg-white p-4 shadow-sm min-h-[120px] cursor-pointer hover:border-emerald-200 transition-colors"><div className={`inline-flex h-10 w-10 items-center justify-center rounded-2xl ${tone}`}><CheckCircle2 className="h-4 w-4" /></div><p className="mt-3 text-[11px] uppercase tracking-wider text-slate-400 font-medium">{label}</p><div className="mt-1 text-[30px] font-bold text-slate-800 leading-none">{value}</div>{sub && <p className="mt-1.5 text-[12px] text-muted-foreground">{sub}</p>}</div></Link>;
}
...
export default function Dashboard() {
  ...
  return <div className="space-y-6 page-enter pb-8">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 items-stretch">
      <Link href="/goals" className="h-full block"><div className="h-full min-h-[230px] rounded-[30px] border border-[#e4ddd2] bg-white p-5 shadow-sm cursor-pointer hover:border-emerald-200 transition-colors"><div className="flex items-center justify-between mb-4"><h2 className="text-[16px] font-semibold text-slate-800">Goals progress</h2><span className="text-[12px] text-slate-400">{summary?.activeGoals ?? 0} active</span></div>{goals.length ? <div className="space-y-4">{goals.slice(0, 3).map((goal) => <div key={goal.id} className="space-y-2"><div className="flex items-center justify-between gap-3"><p className="text-sm font-medium text-slate-700">{goal.title}</p><span className="text-sm text-slate-400">{goal.progress}%</span></div><div className="h-1.5 rounded-full bg-[#f0ebe0] overflow-hidden"><div className="h-full rounded-full bg-emerald-600" style={{ width: `${goal.progress}%` }} /></div></div>)}</div> : <p className="text-sm text-muted-foreground">No goals yet.</p>}</div></Link>
      <Link href="/research" className="h-full block"><div className="h-full min-h-[230px] rounded-[30px] border border-[#e4ddd2] bg-white p-5 shadow-sm cursor-pointer hover:border-emerald-200 transition-colors"><div className="flex items-center justify-between mb-4"><h2 className="text-[16px] font-semibold text-slate-800">Research items</h2><span className="text-[12px] text-slate-400">see all</span></div><div className="space-y-3">{research.slice(0, 3).map((item) => <div key={item.id} className="rounded-[20px] border border-[#ebe5d8] bg-[#fdfcf8] p-4 flex items-start gap-3"><div className="mt-1 h-2.5 w-2.5 rounded-full bg-amber-400" /><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-3"><p className="font-medium text-slate-800 truncate">{item.title}</p><span className="text-xs text-muted-foreground">{item.status.replace(/_/g, " ")}</span></div><p className="text-xs text-muted-foreground mt-1">{item.type} · {item.source ?? "No source"}</p></div></div>)}{research.length === 0 && <p className="text-sm text-muted-foreground">No research items yet.</p>}</div></div></Link>
    </div>

    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 items-stretch">
      <Link href="/progress" className="h-full block"><div className="h-full min-h-[220px] rounded-[30px] border border-[#e4ddd2] bg-white p-5 shadow-sm cursor-pointer hover:border-emerald-200 transition-colors"><div className="flex items-center justify-between mb-4"><h2 className="text-[16px] font-semibold text-slate-800">Recent learning</h2><span className="text-[12px] text-slate-400">14 days</span></div><div className="space-y-3"><div className="flex items-center gap-2 pb-2"><span className="text-xs text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-full">M</span><span className="text-xs text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-full">T</span><span className="text-xs text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-full">W</span><span className="text-xs text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-full">T</span><span className="text-xs text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-full">F</span><span className="text-xs text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-full">S</span><span className="text-xs text-white bg-emerald-600 px-2.5 py-1 rounded-full">S</span></div>{recentProgress.map((entry) => <div key={entry.id} className="flex items-center justify-between border-b border-[#f0ebe0] pb-2.5 last:border-0"><span className="text-[13px] text-slate-600 line-clamp-1">{entry.title}</span><span className="text-[12px] text-emerald-600 shrink-0 ml-2">{entry.durationHours > 0 ? `${entry.durationHours} hr` : entry.status.replace("_", " ")}</span></div>)}</div></div></Link>
      <Link href="/progress" className="h-full block"><div className="h-full min-h-[220px] rounded-[30px] border border-[#e4ddd2] bg-white p-5 shadow-sm cursor-pointer hover:border-emerald-200 transition-colors"><div className="flex items-center justify-between mb-4"><h2 className="text-[16px] font-semibold text-slate-800">Skill gap analyzer</h2><span className="text-[12px] text-slate-400">Goals vs learning vs jobs</span></div>{skillGap ? <div className="space-y-4"><div className="grid grid-cols-3 gap-3"><div className="rounded-[20px] bg-[#fdfcf8] border border-[#ebe5d8] p-4"><p className="text-xs text-slate-400">Coverage</p><p className="text-2xl font-bold text-slate-800">{skillGap.coveragePercent}%</p></div><div className="rounded-[20px] bg-[#fdfcf8] border border-[#ebe5d8] p-4"><p className="text-xs text-slate-400">Gaps</p><p className="text-2xl font-bold text-slate-800">{skillGap.gaps.length}</p></div><div className="rounded-[20px] bg-[#fdfcf8] border border-[#ebe5d8] p-4"><p className="text-xs text-slate-400">Covered</p><p className="text-2xl font-bold text-slate-800">{skillGap.covered.length}</p></div></div><div className="flex flex-wrap gap-2">{skillGap.gaps.slice(0, 8).map((gap: string) => <Badge key={gap} variant="outline" className="text-xs">{gap}</Badge>)}</div></div> : <Skeleton className="h-36 w-full rounded-[24px]" />}</div></Link>
    </div>
    ...
    <div className="rounded-[30px] border border-[#e4ddd2] bg-white p-5 shadow-sm">
      ...
      {loadingJobs ? <div className="grid grid-cols-1 md:grid-cols-5 gap-3">{[1,2,3,4,5].map(i => <Skeleton key={i} className="h-28 rounded-[24px]" />)}</div> : <div className="grid grid-cols-1 md:grid-cols-5 gap-3">{[{ label: "Saved", count: jobs.filter((j) => j.status === "saved").length, icon: Briefcase, tone: "text-slate-600 bg-slate-100" }, { label: "Applied", count: jobs.filter((j) => j.status === "applied").length, icon: Clock3, tone: "text-sky-600 bg-sky-100" }, { label: "Interviewing", count: jobs.filter((j) => j.status === "interviewing").length, icon: Sparkles, tone: "text-amber-600 bg-amber-100" }, { label: "Offered", count: jobs.filter((j) => j.status === "offered").length, icon: CircleCheckBig, tone: "text-emerald-600 bg-emerald-100" }, { label: "Rejected", count: jobs.filter((j) => j.status === "rejected").length, icon: XCircle, tone: "text-rose-600 bg-rose-100" }].map((item) => { const Icon = item.icon; return <Link key={item.label} href="/jobs"><div className="rounded-[24px] border border-[#ebe5d8] bg-[#fdfcf8] p-4 hover:border-emerald-200 transition-colors cursor-pointer min-h-[118px]"><div className={`h-9 w-9 rounded-2xl flex items-center justify-center ${item.tone}`}><Icon className="h-4 w-4" /></div><p className="mt-3 text-[13px] font-medium text-slate-700">{item.label}</p><p className="text-[24px] font-bold text-slate-800 leading-none mt-1">{item.count}</p></div></Link>; })}</div>}
    </div>

    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 items-stretch">
      <Link href="/jobs" className="h-full block"><div className="h-full min-h-[230px] rounded-[30px] border border-[#e4ddd2] bg-white p-5 shadow-sm cursor-pointer hover:border-emerald-200 transition-colors"><div className="flex items-center justify-between mb-4"><h2 className="text-[16px] font-semibold text-slate-800">Pinned items</h2><span className="text-[12px] text-slate-400">jobs</span></div>{pinnedJobs.length > 0 ? <div className="space-y-2.5">{pinnedJobs.slice(0, 2).map((job) => <div key={job.id} className="rounded-[20px] border border-[#ebe5d8] bg-[#fdfcf8] p-4"><p className="font-medium text-slate-800">{job.title}</p><p className="text-xs text-muted-foreground">{job.company ?? "No company"}</p></div>)}</div> : <p className="text-sm text-muted-foreground">Pin jobs to keep them here.</p>}</div></Link>
      <Link href="/jobs" className="h-full block"><div className="h-full min-h-[230px] rounded-[30px] border border-[#e4ddd2] bg-white p-5 shadow-sm cursor-pointer hover:border-emerald-200 transition-colors"><div className="flex items-center justify-between mb-4"><h2 className="text-[16px] font-semibold text-slate-800">Interview prep tracking</h2><span className="text-[12px] text-slate-400">see all</span></div><div className="space-y-3">{(analytics?.topSkills ?? []).slice(0, 4).map((item) => <div key={item.skill} className="flex items-center justify-between rounded-[20px] border border-[#ebe5d8] bg-[#fdfcf8] px-4 py-3"><span className="text-sm text-slate-700">{item.skill}</span><span className="text-sm font-semibold text-emerald-700">{item.count}</span></div>)}{!analytics?.topSkills?.length && <p className="text-sm text-muted-foreground">No interview prep data yet.</p>}</div></div></Link>
    </div>
  </div>;
}
