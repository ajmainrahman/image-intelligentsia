...
export default function Dashboard() {
  ...
  return <div className="space-y-6 page-enter pb-8">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
      <Link href="/jobs"><div className="min-h-[156px] rounded-[30px] border border-[#e4ddd2] bg-white p-5 shadow-sm cursor-pointer hover:border-emerald-200 transition-colors"><div className="flex items-center justify-between mb-4"><h2 className="text-[16px] font-semibold text-slate-800">Pinned items</h2><span className="text-[12px] text-slate-400">jobs</span></div>{pinnedJobs.length > 0 ? <div className="space-y-2.5">{pinnedJobs.slice(0, 4).map((job) => <div key={job.id} className="rounded-[20px] border border-[#ebe5d8] bg-[#fdfcf8] p-4"><p className="font-medium text-slate-800">{job.title}</p><p className="text-xs text-muted-foreground">{job.company ?? "No company"}</p></div>)}</div> : <p className="text-sm text-muted-foreground">Pin jobs to keep them here.</p>}</div></Link>
