...
function StatCard({ label, value, tone, sub }: { label: string; value: string | number; tone: string; sub?: string }) {
  return <div className="rounded-[28px] border border-[#e4ddd2] bg-white p-5 shadow-sm min-h-[132px]"><div className={`inline-flex h-11 w-11 items-center justify-center rounded-2xl ${tone}`}><CheckCircle2 className="h-5 w-5" /></div><p className="mt-4 text-[11px] uppercase tracking-wider text-slate-400 font-medium">{label}</p><div className="mt-1 text-[34px] font-bold text-slate-800 leading-none">{value}</div>{sub && <p className="mt-2 text-[12px] text-muted-foreground">{sub}</p>}</div>;
}
...
export default function Dashboard() {
  ...
  return <div className="space-y-6 page-enter pb-8">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
      <div className="rounded-[32px] border border-[#e4ddd2] bg-white p-6 shadow-sm">
        <div className="flex items-center justify-between mb-5"><h2 className="text-[16px] font-semibold text-slate-800">Pinned items</h2><Link href="/jobs" className="text-[12px] text-slate-400 hover:text-emerald-600 transition-colors">jobs</Link></div>
