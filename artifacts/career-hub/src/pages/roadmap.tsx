...
type RoadmapItem = {
  ...
  yearTarget: number; phase: Phase; status: Status;
  goalId: number | null; order: number;
};
...
const PHASES = [
  { id: "short_term" as Phase, label: "Short Term", short: "Short", pillBg: "bg-rose-50",  pillText: "text-rose-700",  colBorder: "border-rose-100",  colHeader: "text-rose-700" },
  { id: "mid_term"   as Phase, label: "Mid Term",   short: "Mid",   pillBg: "bg-amber-50", pillText: "text-amber-700", colBorder: "border-amber-100", colHeader: "text-amber-700" },
  { id: "long_term"  as Phase, label: "Long Term",  short: "Long",  pillBg: "bg-accent",   pillText: "text-primary",   colBorder: "border-border",    colHeader: "text-primary" },
];
...
function SortableCard({ id, children }: { id: number; children: (dragHandle: React.ReactNode) => React.ReactNode }) {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id });
  const style = { transform: CSS.Transform.toString(transform), transition, opacity: isDragging ? 0.5 : 1 };
  const handle = (
    <button {...attributes} {...listeners} className="cursor-grab active:cursor-grabbing p-1 rounded text-muted-foreground/40 hover:text-muted-foreground transition-colors touch-none">
...
export default function RoadmapPage() {
  ...
  const { data: items = [], isLoading } = useQuery<RoadmapItem[]>({
    queryKey: ["roadmap"], queryFn: () => api<RoadmapItem[]>("/roadmap"),
  });
  ...
  const createItem = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["roadmap"] }); queryClient.invalidateQueries({ queryKey: ["activity"] }); closeDialog(); toast({ title: "Milestone added" }); },
    onError: (e: Error) => toast({ title: "Error", description: e.message, variant: "destructive" }),
  });
  const updateItem = useMutation({
    mutationFn: ({ id, data }: { id: number; data: object }) => api(`/roadmap/${id}`, { method: "PUT", body: JSON.stringify(data) }),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["roadmap"] }); closeDialog(); toast({ title: "Milestone updated" }); },
    onError: (e: Error) => toast({ title: "Error", description: e.message, variant: "destructive" }),
  });
  const deleteItem = useMutation({
    mutationFn: (id: number) => api(`/roadmap/${id}`, { method: "DELETE" }),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["roadmap"] }); toast({ title: "Milestone deleted" }); },
    onError: (e: Error) => toast({ title: "Error", description: e.message, variant: "destructive" }),
  });
  const reorderMutation = useMutation({
    mutationFn: (orders: { id: number; order: number }[]) =>
      api("/roadmap/reorder", { method: "PATCH", body: JSON.stringify(orders) }),
    onError: () => { queryClient.invalidateQueries({ queryKey: ["roadmap"] }); toast({ title: "Reorder failed", variant: "destructive" }); },
  });
  ...
  const updateResponsibility = (i: number, value: string) => setForm((f) => ({ ...f, responsibilities: f.responsibilities.map((r, idx) => idx === i ? value : r) }));
  const removeResponsibility = (i: number) => setForm((f) => ({ ...f, responsibilities: f.responsibilities.filter((_, idx) => idx !== i) }));

  const addSkill = (raw: string) => {
    const skill = raw.trim().replace(/,$/, "");
    if (!skill || form.skills.some((s) => s.toLowerCase() === skill.toLowerCase())) { setForm((f) => ({ ...f, skillDraft: "" })); return; }
  ...
  const itemsByPhase = useMemo(() => {
    ...
    for (const phase of Object.keys(buckets) as Phase[]) {
      buckets[phase].sort((a, b) => a.order - b.order || a.yearTarget - b.yearTarget);
    }
    return buckets;
  }, [items]);

  function handleDragEnd(phase: Phase, event: DragEndEvent) {
    const { active, over } = event;
    if (!over || active.id === over.id) return;
    const list = itemsByPhase[phase];
    const oldIndex = list.findIndex((i) => i.id === active.id);
    const newIndex = list.findIndex((i) => i.id === over.id);
    const reordered = arrayMove(list, oldIndex, newIndex);
    const orders = reordered.map((item, idx) => ({ id: item.id, order: idx }));
    // Optimistic update
    queryClient.setQueryData<RoadmapItem[]>(["roadmap"], (prev = []) => {
      const updated = [...prev];
            ...
            <div className="space-y-4 pt-2">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Input value={form.title} onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))} placeholder="e.g. Senior ML Engineer" className="bg-secondary border-border text-[13px]" />
                </div>
                <div className="space-y-1.5">
                  <label className="text-[12px] font-medium text-muted-foreground">Target Year</label>
                  <Input type="number" value={form.yearTarget} onChange={(e) => setForm((f) => ({ ...f, yearTarget: e.target.value }))} className="bg-secondary border-border text-[13px]" />
                </div>
              </div>
              <div className="space-y-1.5">
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-[10px] text-muted-foreground px-0.5">{LEVEL_LABELS.map((l) => <span key={l}>{l}</span>)}</div>
              </div>
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  <label className="text-[12px] font-medium text-muted-foreground">Focus On</label>
                  <span className={`text-[11px] ${form.focusOn.length > MAX_FOCUS * 0.9 ? "text-amber-500" : "text-muted-foreground"}`}>{form.focusOn.length}/{MAX_FOCUS}</span>
                </div>
                <Textarea rows={5} value={form.focusOn} onChange={(e) => setForm((f) => ({ ...f, focusOn: e.target.value.slice(0, MAX_FOCUS) }))} placeholder="What's the primary focus at this stage?" className="resize-y bg-secondary border-border text-[13px] min-h-[120px]" />
              </div>
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <label className="text-[12px] font-medium text-muted-foreground">Key Responsibilities</label>
                  <button type="button" onClick={addResponsibility} className="text-[11px] text-primary hover:underline flex items-center gap-1"><Plus className="h-3 w-3" /> Add</button>
                </div>
                <div className="space-y-2">
                  {form.responsibilities.length === 0 && <p className="text-[12px] text-muted-foreground">No responsibilities yet.</p>}
                  {form.responsibilities.map((r, i) => (
                    <div key={i} className="flex items-center gap-2">
                      <Input value={r} onChange={(e) => updateResponsibility(i, e.target.value)} placeholder={`Responsibility ${i + 1}`} className="bg-secondary border-border text-[13px] flex-1" />
                      <button type="button" onClick={() => removeResponsibility(i)} className="p-1.5 rounded-lg hover:bg-red-50 text-muted-foreground hover:text-red-500 transition-colors"><X className="h-3.5 w-3.5" /></button>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="flex flex-wrap items-center gap-1.5 rounded-lg border border-border bg-secondary p-2 min-h-[44px]">
                  {form.skills.map((skill) => (
                    <span key={skill} className="flex items-center gap-1 text-[11px] px-2 py-0.5 rounded-full bg-accent text-primary">
                      {skill}
                      <button type="button" onClick={() => setForm((f) => ({ ...f, skills: f.skills.filter((s) => s !== skill) }))} className="hover:text-foreground transition-colors"><X className="h-2.5 w-2.5" /></button>
                    </span>
                  ))}
                  <input value={form.skillDraft} onChange={(e) => setForm((f) => ({ ...f, skillDraft: e.target.value }))} onKeyDown={onSkillKeyDown} onBlur={() => addSkill(form.skillDraft)} placeholder={form.skills.length === 0 ? "Type a skill, press Enter…" : "Add more…"} className="flex-1 min-w-[120px] bg-transparent outline-none text-[13px] text-foreground placeholder:text-muted-foreground" />
                </div>
              </div>
              <div className="space-y-2">
                <div className="flex items-center justify-between">
          ...
          return (
            <div key={phase.id} className={`rounded-2xl border bg-card p-5 flex flex-col ${phase.colBorder}`}>
              ...
              {isLoading ? (
                ...
                <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={(e) => handleDragEnd(phase.id, e)}>
                    ...
                    <div className="space-y-3">
                      {list.map((item, index) => (
                        <SortableCard key={item.id} id={item.id}>
                          {(dragHandle) => (
                            <RoadmapCard
                              item={item} phase={phase} index={index} goals={goals}
...
function RoadmapCard({ item, phase, index, goals, expanded, onToggleExpand, onEdit, onDelete, dragHandle }: {
  item: RoadmapItem; phase: typeof PHASES[number]; index: number; goals: Goal[];
  ...
  return (
    <motion.article initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div className="absolute top-3 right-3 flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
        <button onClick={onEdit} className="p-1.5 rounded-lg hover:bg-secondary text-muted-foreground hover:text-foreground transition-colors"><Pencil className="h-3 w-3" /></button>
        <button onClick={onDelete} className="p-1.5 rounded-lg hover:bg-red-50 text-muted-foreground hover:text-red-500 transition-colors"><Trash2 className="h-3 w-3" /></button>
      </div>
      ...
      {linkedGoal && <div className="mb-2"><span className="text-[10px] px-2 py-0.5 rounded-full bg-accent text-primary">↗ {linkedGoal.targetRole}</span></div>}

      {focusOn && (
        <div className="mb-3">
          <p className={`text-[12px] text-muted-foreground leading-relaxed ${!expanded && focusLong ? "line-clamp-2" : ""}`}>{focusOn}</p>
          {focusLong && (
      ...
      {structured?.skills && structured.skills.length > 0 && (
        <div className="pt-3 border-t border-border flex flex-wrap gap-1.5">
          {structured.skills.map((skill) => <span key={skill} className="text-[10px] px-2 py-0.5 rounded-full bg-accent text-primary">{skill}</span>)}
        </div>
      )}
    </motion.article>
  );
