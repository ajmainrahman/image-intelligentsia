...
const PHASES: {
...
}[] = [
  {
    ...
    colBorder: "border-rose-100",
    colHeader: "text-rose-700",
  },
  {
    id: "mid_term",
    label: "Mid Term",
...
export default function RoadmapPage() {
  ...
  const openCreate = (phase: Phase = "short_term") => {
    setForm(emptyForm(phase)); setEditingId(null); setOpen(true);
  };
  ...
  const submit = () => {
    ...
    const payload = {
      ...
      phase: form.phase, status: form.status, description, order: 0,
    };
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-[14px] text-muted-foreground mt-1.5">
            Your 1–10 year trajectory, broken into phases.
          </p>
        </div>
            ...
            <div className="space-y-4 pt-2">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[12px] font-medium text-primary">
                    Level {form.level} · {LEVEL_LABELS[form.level - 1]}
                  </span>
                </div>
                <Slider
                  value={[form.level]}
              ...
              <div className="space-y-1.5">
                ...
                <Textarea
                  ...
                  placeholder="What's the primary focus at this stage?"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        {PHASES.map((phase) => {
          ...
          return (
            <div
              key={phase.id}
              className={`rounded-2xl border bg-card p-5 flex flex-col ${phase.colBorder}`}
            >
              {/* Column header */}
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  <h2 className={`text-[14px] font-medium ${phase.colHeader}`} style={serif}>
                    {phase.label}
                  </h2>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-1.5 pr-14 mb-2">
        <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${phase.pillBg} ${phase.pillText}`}>
          {phase.short}
        </span>
      ...
      {!structured && legacyText && (
        <p className="text-[12px] text-muted-foreground mb-3 leading-relaxed line-clamp-3">
          {legacyText}
        </p>
      )}
      ...
      {structured?.skills && structured.skills.length > 0 && (
        <div className="pt-3 border-t border-border flex flex-wrap gap-1.5">
          {structured.skills.map((skill) => (
            <span
              key={skill}
              className="text-[10px] px-2 py-0.5 rounded-full bg-accent text-primary"
            >
              {skill}
            </span>
          ))}
