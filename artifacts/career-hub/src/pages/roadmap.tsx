...
import { useListRoadmapItems, getListRoadmapItemsQueryKey, useCreateRoadmapItem, useUpdateRoadmapItem, useDeleteRoadmapItem } from "@workspace/api-client-react";
...
export default function RoadmapPage() {
  const { data: roadmapItems, isLoading } = useListRoadmapItems({ query: { queryKey: getListRoadmapItemsQueryKey() } });
  ...
  const updateRoadmapItem = useUpdateRoadmapItem({
    mutation: {
      onSuccess: () => {
        ...
        toast({ title: "Milestone updated", description: "Roadmap milestone saved." });
      },
    }
  });

  const deleteRoadmapItem = useDeleteRoadmapItem({
    mutation: {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: getListRoadmapItemsQueryKey() });
        toast({ title: "Milestone deleted", description: "The milestone has been removed from your roadmap." });
      },
    }
  });
  ...
  const onSubmit = (data: RoadmapItemFormValues) => {
    ...
    if (editingItemId) {
      ...
      createRoadmapItem.mutate({ data: payload });
    }
  };
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      default: return <Circle className="h-5 w-5 text-slate-300 dark:text-slate-600" />;
    }
  };
  ...
  // Sort items by year target and order
  Object.keys(itemsByPhase).forEach(phase => {
    itemsByPhase[phase].sort((a, b) => {
      if (a.yearTarget !== b.yearTarget) return a.yearTarget - b.yearTarget;
  ...
  return (
    <div className="space-y-8 page-enter max-w-4xl mx-auto">
      <div className="flex items-center justify-between">
        ...
        <Dialog open={isCreateOpen || editingItemId !== null} onOpenChange={(open) => {
          if (!open) {
            setIsCreateOpen(false);
            setEditingItemId(null);
            form.reset();
            ...
            form.setValue("phase", activePhase);
          }
        }}>
          ...
          <DialogContent className="sm:max-w-[500px]">
            <DialogHeader>
              <DialogTitle>{editingItemId ? "Edit Milestone" : "Add Milestone"}</DialogTitle>
            </DialogHeader>
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-4">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="e.g. Master Deep Learning" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <div className="flex justify-end pt-4">
                  <Button 
                    type="submit" 
                    disabled={createRoadmapItem.isPending || updateRoadmapItem.isPending}
                  >
      ...
      <div className="flex space-x-1 p-1 bg-muted rounded-lg w-full mb-8">
        {phases.map((phase) => (
          <button
            ...
            className={`flex-1 flex items-center justify-center py-2.5 px-4 text-sm font-medium rounded-md transition-all
      ...
      <div className="bg-card border rounded-xl p-6 shadow-sm">
        ...
        {isLoading ? (
        ...
        ) : itemsByPhase[activePhase] && itemsByPhase[activePhase].length > 0 ? (
              ...
              {itemsByPhase[activePhase].map((item) => (
                <div key={`roadmap-${item.id}`} className="flex gap-4 group">
                  ...
                  <div className="flex-1 border rounded-lg p-4 bg-background shadow-sm hover:border-primary/30 transition-colors relative">
                    <div className="absolute top-4 right-4 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-destructive" onClick={() => {
                        if (confirm("Are you sure you want to delete this milestone?")) {
                          deleteRoadmapItem.mutate({ id: item.id });
                        }
                      }}>
                    ...
                    <Badge variant="secondary" className="mb-2 bg-primary/10 text-primary hover:bg-primary/20 font-medium">
                      Target: {item.yearTarget}
                    </Badge>
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center border-2 border-dashed rounded-xl bg-muted/10">
            ...
            <p className="text-muted-foreground text-sm max-w-sm mb-6">
              Plan out the key achievements you need to reach your career goals during this time period.
            </p>
            <Button onClick={() => {
              form.setValue("phase", activePhase);
              setIsCreateOpen(true);
