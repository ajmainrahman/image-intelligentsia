...
import { Microscope, Plus, ExternalLink, Pencil, Trash2, Search, FileText, BookOpen, GraduationCap, Database, Lightbulb, StickyNote, Library } from "lucide-react";
...
const EMPTY_FORM: ResearchFormState = {
  ...
  summary: "", tagsText: "", status: "to_explore", notes: "",
};

const TYPE_META: Record<ResearchType, { label: string; Icon: React.ElementType }> = {
...
const STATUS_META: Record<ResearchStatus, { label: string; chip: string }> = {
  to_explore: { label: "To explore", chip: "bg-amber-100 text-amber-800 dark:bg-amber-500/15 dark:text-amber-300" },
  reading:    { label: "Reading",    chip: "bg-sky-100 text-sky-800 dark:bg-sky-500/15 dark:text-sky-300" },
  working:    { label: "Working",    chip: "bg-violet-100 text-violet-800 dark:bg-violet-500/15 dark:text-violet-300" },
  completed:  { label: "Completed",  chip: "bg-emerald-100 text-emerald-800 dark:bg-emerald-500/15 dark:text-emerald-300" },
};

function toForm(item: ResearchItem): ResearchFormState {
  return { title: item.title, type: item.type, authors: item.authors ?? "", source: item.source ?? "",
    summary: item.summary ?? "", tagsText: item.tags.join(", "), status: item.status, notes: item.notes ?? "" };
}

function payloadFromForm(form: ResearchFormState) {
  return { title: form.title.trim(), type: form.type, authors: form.authors.trim() || null,
...
export default function ResearchPage() {
  ...
  return (
    <div className="page-enter space-y-6">
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h1 className="text-2xl sm:text-[28px] font-bold text-foreground tracking-tight">Research</h1>
            <p className="text-sm text-muted-foreground mt-0.5">Capture papers, articles, and ideas — and tie them to your goals.</p>
          </div>
        </div>
      ...
      {/* Stats strip — now 5 columns with Working */}
      ...
      <div className="space-y-3">
        <div className="relative">
          ...
          <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search title, authors, tags…" className="pl-9 h-11" />
        </div>
...
function StatTile({ label, value, highlight }: { label: string; value: number; highlight?: boolean }) {
  return (
    <div className={`rounded-xl border px-3.5 py-3 ${highlight ? "border-violet-200 bg-violet-50 dark:border-violet-500/20 dark:bg-violet-500/10" : "border-border bg-card"}`}>
      <div className={`text-[11px] uppercase tracking-wider font-medium ${highlight ? "text-violet-600 dark:text-violet-400" : "text-muted-foreground"}`}>{label}</div>
      <div className={`text-2xl font-bold mt-0.5 leading-none ${highlight ? "text-violet-700 dark:text-violet-300" : "text-foreground"}`}>{value}</div>
    </div>
  );
}
...
function Chip({ children, active, onClick }: { children: React.ReactNode; active?: boolean; onClick?: () => void }) {
  return (
    <button type="button" onClick={onClick}
      className={`px-3 py-1.5 rounded-full text-[12.5px] font-medium border transition-colors ${active ? "bg-primary text-primary-foreground border-primary" : "bg-card text-muted-foreground border-border hover:text-foreground hover:border-foreground/30"}`}>
      {children}
    </button>
  );
}
  ...
  return (
    <Card className="card-hover">
      <CardHeader className="pb-2">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              <CardTitle className="text-[15.5px] leading-snug break-words">{item.title}</CardTitle>
      ...
      <CardContent className="pt-0">
        {item.summary && <p className="text-[13.5px] text-muted-foreground leading-relaxed line-clamp-3">{item.summary}</p>}
        <div className="flex flex-wrap items-center gap-1.5 mt-3">
          <span className={`text-[11px] font-medium px-2 py-0.5 rounded-full ${status.chip}`}>{status.label}</span>
          {item.tags.slice(0, 6).map((tag) => <Badge key={tag} variant="secondary" className="text-[10.5px] font-normal">{tag}</Badge>)}
        </div>
      </CardContent>
    </Card>
  );
...
function EmptyState({ onAdd, hasItems }: { onAdd: () => void; hasItems: boolean }) {
  return (
    <div className="rounded-2xl border border-dashed border-border bg-card/50 px-6 py-14 flex flex-col items-center text-center">
      ...
      <h3 className="text-lg font-semibold text-foreground">{hasItems ? "Nothing matches those filters" : "Start your research library"}</h3>
      ...
      {!hasItems && <Button onClick={onAdd} className="mt-5"><Plus className="h-4 w-4" />Add your first item</Button>}
    </div>
  );
}
    ...
    <DialogContent className="sm:max-w-lg max-h-[92dvh] overflow-y-auto">
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <Textarea id="r-summary" value={form.summary} onChange={(e) => setForm({ ...form, summary: e.target.value })} placeholder="What's it about? Key findings?" rows={3} />
        </div>
        <div className="space-y-1.5">
        ...
        <div className="space-y-1.5">
          ...
          <Textarea id="r-notes" value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} placeholder="Personal takeaways, citations to follow up…" rows={3} />
        </div>
        <DialogFooter className="gap-2 sm:gap-2">
          <Button type="submit" disabled={saving} className="h-11 sm:h-10">{saving ? "Saving…" : editing ? "Save changes" : "Add to library"}</Button>
        </DialogFooter>
      </form>
    </DialogContent>
  );
