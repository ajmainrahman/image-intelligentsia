...
import {
  CalendarDays, NotebookPen, Plus, Save, Trash2, Search,
  Hash, Sparkles, ArrowUpDown, Lightbulb,
...
const QUICK_TAGS = [
  "planning", "research", "building", "reflection",
  "design", "writing", "feedback", "milestone", "problem", "win",
];
...
function extractTags(note: Note): string[] {
  const fromTitle = note.title.match(/#[\w-]+/g) ?? [];
  return [...new Set([...fromTitle, ...extractTagsFromHtml(note.content)].map((t) => t.slice(1).toLowerCase()))];
}
...
function Toolbar({ editor }: { editor: ReturnType<typeof useEditor> }) {
  ...
  return (
    <div className="flex items-center gap-0.5 flex-wrap border-b border-border/60 pb-2 mb-1">
      ...
      {btn(editor.isActive("underline"), () => editor.chain().focus().toggleUnderline().run(), <Underline className="h-3.5 w-3.5" />, "Underline")}
...
function NoteEditor({
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: [
          "min-h-[420px] px-1 py-2 text-[15px] leading-7 text-foreground outline-none",
  ...
  // Sync external content changes (note switching)
  const prevIdRef = useRef<string>("");
  useEffect(() => {
    if (!editor) return;
    const sig = content.slice(0, 40);
    if (prevIdRef.current !== sig) {
      prevIdRef.current = sig;
...
export default function NotepadPage() {
  ...
  const allTags = useMemo(() => {
    ...
    notes.forEach((n) => extractTags(n).forEach((t) => tags.add(t)));
    return [...tags].sort();
  }, [notes]);
  ...
  const filteredNotes = useMemo(() => {
    ...
    const base = notes.filter((n) => {
      ...
      const matchesQ = !q || n.title.toLowerCase().includes(q) || plain.toLowerCase().includes(q) || tags.some((t) => t.includes(q));
  ...
  const insertTag = (tag: string) => {
    const sep = draftContent.trim() && !draftContent.trim().endsWith("<br>") ? "<br>" : "";
    persist({ content: `${draftContent}${sep}<p>#${tag}</p>` });
  };
        ...
        <Card className="border-primary/15 bg-card/95 shadow-sm">
          ...
          {selectedNote ? (
            <CardContent className="space-y-4">
              {/* Prompt banner */}
              <div className="rounded-2xl border border-border/60 bg-muted/20 px-4 py-3 flex items-start justify-between gap-3">
                <button
                  type="button"
                ...
                >
                  ...
                  {showPrompt ? prompt : "Need inspiration? See today's prompt"}
                </button>
                <Popover>
                  ...
                  <PopoverContent className="w-72">
                    <p className="text-sm font-medium mb-2">Insert a tag</p>
              ...
              <div className="flex gap-3">
                ...
                />
                <Button type="button" variant="outline" onClick={toggleDelete}>
            ...
            <CardContent>
              <div className="py-16 text-center">
                <NotebookPen className="h-10 w-10 mx-auto text-muted-foreground mb-3" />
                <p className="text-muted-foreground text-sm">No note selected.</p>
                <Button onClick={addNote} className="mt-4 gap-2" variant="outline">
