...
// Returns the current authenticated user — used by the frontend on startup
// to validate a stored session cookie.
router.get("/auth/me", requireAuth, async (req: AuthRequest, res, next): Promise<void> => {
    try {
      const [user] = await db
