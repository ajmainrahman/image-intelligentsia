...
const ResearchBody = z.object({
  ...
  status: z.enum(["to_explore", "reading", "completed"]).default("to_explore"),
...
router.get("/research", requireAuth, async (req: AuthRequest, res, next): Promise<void> => {
  try {
    const items = await db
      .select()
      .from(researchTable)
      .where(eq(researchTable.userId, req.userId!))
...
router.put("/research/:id", requireAuth, async (req: AuthRequest, res, next): Promise<void> => {
  try {
    ...
    const [item] = await db
      ...
      .where(and(eq(researchTable.id, id), eq(researchTable.userId, req.userId!)))
      .returning();
    if (!item) {
